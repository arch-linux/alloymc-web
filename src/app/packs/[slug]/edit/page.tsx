...
export default function EditPackPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setForm({
        ...
        longDescription: data.pack.longDescription || "",
        githubRepo: data.pack.githubRepo || "",
        category: data.pack.category,
        useGithubReadme: data.pack.useGithubReadme,
      });
      setIconUrl(data.pack.iconUrl);
      setWebhookUrl(`${window.location.origin}/api/webhooks/github-pack/${slug}`);
      setLoading(false);
    }
    if (!authLoading) load();
  ...
  async function handleIconUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/packs/${slug}/images`, {
        method: "POST",
        body: formData,
      });
  ...
  async function handleRegenSecret() {
    ...
    try {
      const res = await fetch(`/api/packs/${slug}/webhook-secret`, { method: "POST" });
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const data = (await res.json()) as any;
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen grid-overlay py-16">
      <div className="mx-auto max-w-2xl px-6">
        <div className="mb-8">
          <h1 className="font-heading text-3xl font-bold text-stone-100">Edit Pack</h1>
          <p className="text-stone-400 mt-2">Update your pack&apos;s information.</p>
        </div>
