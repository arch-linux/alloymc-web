...
export function CommentSection({ slug, modOwnerId }: CommentSectionProps) {
  const { user } = useAuth();
  ...
  const fetchComments = useCallback(async () => {
    try {
      const res = await fetch(`/api/mods/${slug}/comments`);
      if (res.ok) {
  ...
  async function handleDelete(commentId: number) {
    const res = await fetch(`/api/mods/${slug}/comments/${commentId}`, {
      method: "DELETE",
    });
    if (res.ok) {
        ...
        <div className="space-y-4">
          {comments.map((comment) => (
            <CommentItem
              ...
              {...comment}
              canDelete={
                user?.userId === comment.userId || user?.userId === modOwnerId
              }
